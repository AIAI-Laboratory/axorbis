...
function makeAppRoot(): string {
	...
	writeFileSync(join(root, "dist", "workbench-web", "research.html"), [
		"<!doctype html>",
		"<html><head><title>Feynman Research Workspace</title></head>",
		"<body><div id=\"root\"></div><script type=\"module\" src=\"/app-shell/assets/research.js\"></script></body></html>",
	].join(""));
...
test("workbench server serves the authenticated React app shell on default and app-shell routes", async () => {
	...
	try {
		...
		assert.match(await shell.text(), /\/app-shell\/assets\/research\.js/);

		const launcher = await fetch(`${handle.url}?token=secret`);
		assert.equal(launcher.status, 200);
		assert.match(await launcher.text(), /\/app-shell\/assets\/research\.js/);
		...
		assert.match(await appShellAlias.text(), /\/app-shell\/assets\/research\.js/);
