...
const piAi=await import(pathToFileURL(path.join(root,'.axorbis/npm/node_modules/@earendil-works/pi-ai/dist/index.js')));
const {Type}=await import(pathToFileURL(path.join(root,'.axorbis/npm/node_modules/typebox/build/index.mjs')));
...
const proof={codingAgentEntry:path.join(root,'.axorbis/npm/node_modules/@earendil-works/pi-coding-agent/dist/index.js'),subagents:JSON.parse(fs.readFileSync(path.join(sub,'package.json'))).version};
