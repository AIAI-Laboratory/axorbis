...
export function formatWorkbenchRuntimeContextForPrompt(
	...
	const lines = [
		...
		"- Scientific model endpoints are executable through feynman_model_endpoint_call when their status below is not disabled. Hosted ESMFold uses NVIDIA_API_KEY and self-hosted AlphaFold2 NIM uses endpointUrl; endpoint outputs are saved under .axorbis/artifacts/model-endpoints with provenance sidecars.",
