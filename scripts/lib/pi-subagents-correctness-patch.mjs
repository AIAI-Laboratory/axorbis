...
const MODEL_VERIFICATION_HELPER = [
	...
	"\ttry {",
	"\t\tconst aliases = JSON.parse(process.env.AXORBIS_SUBAGENT_MODEL_ALIASES ?? \"{}\") as Record<string, unknown>;",
	"\t\tconst expectedBase = stripSubagentModelThinkingSuffix(expectedModel);",
	"\t\tconst candidates = aliases[expectedModel] ?? aliases[expectedBase];",
	"\t\tif (Array.isArray(candidates) && candidates.includes(observedIdentity)) return undefined;",
	"\t} catch {",
	"\t\t// Ignore malformed optional alias metadata and keep strict verification.",
	"\t}",
